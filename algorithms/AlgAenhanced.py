...
def length(tour): # DEFINE FITNESS 1/len(tour)
    length = 0
    for i in range(tourSize):
        length += dist_matrix[tour[i]][tour[i + 1]]
    length += dist_matrix[tour[tourSize]][tour[0]]
    return length

def fitness(tour): # DEFINE FITNESS 1/len(tour)
    length =0
    for i in range(tourSize):
        length += dist_matrix[tour[i]][tour[i + 1]]
    length += dist_matrix[tour[tourSize]][tour[0]]
...
def selection(population, weights): # PROBABILITY PROPORTIONAL TO FITNESS ROULETTE WHEEL
    x = random.choices(population, weights)[0]
    while True:
        y = random.choices(population, weights)[0]
        if y != x:
            break
    return x,y
...
# MUTATE BY RANDOMLY SWAPPING 2 CITIES
def mutate(z):
    swap1 = random.randint(0, tourSize)
    swap2 = random.randint(0, tourSize)
    if swap1 != swap2:
        z[swap1], z[swap2] = z[swap2], z[swap1]
    return z

# CREATE VAXINE BY GREEDLY ADDING CITIES
def findVaxine(dist_matrix, vaxineLength):
    shortest = dist_matrix[0][1] ## FIND SHORTEST EDGE IN GRAPH
    vaxine = [0,1]
    for i in range(num_cities):
        for j in range(num_cities):
            if dist_matrix[i][j] != 0 and dist_matrix[i][j] < shortest:
                shortest = dist_matrix[i][j]
                vaxine = [i,j]
    for i in range(vaxineLength-1): ## ADD SHORTEST EDGE TO EITHER FRONT OR BACK OF TOUR
        rightExpand = 10**300
        leftExpand = 10**300
        for j in range(num_cities):
            if dist_matrix[vaxine[-1]][j] !=0 and j not in vaxine and dist_matrix[vaxine[-1]][j] <rightExpand:
                rightExpand = dist_matrix[vaxine[-1]][j]
                cityR = j
            if dist_matrix[vaxine[0]][j] !=0 and j not in vaxine and dist_matrix[vaxine[0]][j] <leftExpand:
                leftExpand = dist_matrix[vaxine[0]][j]
                cityL = j

        if leftExpand > rightExpand:
            vaxine.insert(0,cityR)
        else:
            vaxine.append(cityL)
    return vaxine
    
def insertVaxine(z, vaxine):
    i = 0
    while True:
        if z[i] in vaxine:
            vaxinePoint = i ## FIND FIRST APEARENCE OF VACCINE
            break
        else:
            i+=1
    vaxinated = z.copy()
    vaxinated = vaxinated[:vaxinePoint]+vaxine
    for i in range(vaxinePoint,num_cities):
        if z[i] not in vaxine:
            vaxinated.append(z[i])
    return z, vaxinated
...
if num_cities > 10:
    ...
    vaxineLength =2
changeVaccineChance = 0.5
changeImuneChance = 0.1

imuneChance = 1/populationSize
vaxine = findVaxine(dist_matrix, vaxineLength)

## CREATE RANDOM INITAL POPULATION
population = []
randomTour = list(range(num_cities))
for i in range(populationSize):
    random.shuffle(randomTour)
    newTour = randomTour.copy()
...
while time.time()-start < 59:
    newPopulation = []
    lens = list(length(element) for element in population)
    weights = list(fitness(element) for element in population)
    minLength = min(lens)
    minLengthPosision = lens.index(minLength)
    bestTour = population[minLengthPosision]
    if minLength < newBestLength: ## SETS BEST LENGTH IF BEST SEEN SO FAR
        newBestLength = minLength
        newBestTour = bestTour
        newBestTime = time.time()

    for j in range(populationSize):
        x, y = selection(population, weights) ## SELECT PARENTS BASED ON FITNESS (WEIGHTS)
        z = crossover(x,y)
        if (random.random()<mutationChance):
            mutate(z) ## RANDOMLY MUTATE

        fitZ = fitness(z)
        fitBest = 1/newBestLength
        vaccineChance = changeVaccineChance*((fitBest-fitZ)/fitBest)
        if random.random()<vaccineChance: ## GIVEN PROBABILITY OF vaccineChance
            z, vaxinated = insertVaxine(z, vaxine)
            fitVax = fitness(vaxinated)
            imuneChance = changeImuneChance*((fitBest-fitVax)/fitBest)
            if fitVax>fitZ and random.random()>imuneChance: ## sucessful vaxine and not imune
                z = vaxinated

        newPopulation.append(z)
    population = newPopulation


tour = newBestTour
tour_length = newBestLength
