...
def updatePheromone():
    ...
    for i in range(num_cities):
        phermonSum.append([0]*num_cities)
    for ant in ants:
        if ant.validTour:
            phermonSum = ant.addTourPheromone(phermonSum) ## ADDS INNDIVUAL PHERMON FROM EACH TOUR
            for i in range(num_cities):
                for j in range(num_cities):
...
## f = open("compareAC017.txt", "a")
## f.write("initalPheromone,alpha,beta,p,numants,time,best\n")

## parameters = [[0.9,0.95,1,1.05,1.1],[2,2.3,2.6,3,3.3,3.6,4,4.3,4.6,5,5.3,5.6,6],[0.4,0.45,0.5,0.55,0.6],[5,10,50,100,200],[initalPheromone]]
## for initalPheromone in parameters[4]:
...
###### USED IN PARAMETER TUNING
## compare = str(initalPheromone)+ ','+str(alpha)+ ','+ str(beta)+ ','+ str(p)+ ','+ str(numAnts)+ ','+str(bestTime-start)+ ','+ str(tour_length)
## print(compare)
## f.write(compare+"\n")
## f.close()
